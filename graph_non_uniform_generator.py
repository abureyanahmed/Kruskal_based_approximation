...
j = 0
for e in edges:
        # different weight for every level
        arr = []
        for l in range(levels):
                ...
                if l==0:
                        ...
                        wgt = arr[l-1] + random.randint(1,10)
                arr.append(wgt)
        edge_weights.append(arr)
        print(str(e[0])+" "+str(e[1])+" "+str(edge_weights[j]))
        j = j+1
#print(str(levels))
for l in range(levels):
...
for e in edges:
        s = ""
        for l in range(levels):
                s += str(edge_weights[j][l]) + " "
        file.write(str(e[0])+" "+str(e[1])+" "+s+"\n")
        j = j+1
        #if class_of_graph!=3:
